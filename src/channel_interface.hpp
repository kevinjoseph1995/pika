...
struct Endpoint {
    ...
    auto GetState() const -> EndpointState
    {
        return m_state;
    }
...
struct ProducerImpl : public Endpoint {
    virtual auto Send(uint8_t const* const source_buffer, uint64_t size) -> std::expected<void, PikaError> = 0;
};

struct ConsumerImpl : public Endpoint {
    virtual auto Receive(uint8_t* const destination_buffer, uint64_t destination_buffer_size) -> std::expected<void, PikaError> = 0;
};
...
struct Channel {
    static auto __CreateProducerImpl(ChannelParameters const& channel_params, uint64_t element_size, uint64_t element_alignment) -> std::expected<std::unique_ptr<ProducerImpl>, PikaError>;
    static auto __CreateConsumerImpl(ChannelParameters const& channel_params, uint64_t element_size, uint64_t element_alignment) -> std::expected<std::unique_ptr<ConsumerImpl>, PikaError>;

    template<typename DataT>
    static auto CreateProducer(ChannelParameters const& channel_params) -> std::expected<Producer<DataT>, PikaError>
    {
    ...
    template<typename DataT>
    static auto CreateConsumer(ChannelParameters const& channel_params) -> std::expected<Consumer<DataT>, PikaError>
    {
