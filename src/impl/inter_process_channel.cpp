...
[[nodiscard]] static auto GetBufferSize(pika::ChannelParameters const& channel_parameters, uint64_t element_size, uint64_t element_alignment) -> uint64_t
{
    return GetRingBufferSlotsOffset(element_alignment) + (channel_parameters.queue_size * element_size);
}

auto InterProcessConsumer::Create(pika::ChannelParameters const& channel_params, uint64_t element_size, uint64_t element_alignment) -> std::expected<std::unique_ptr<InterProcessConsumer>, PikaError>
{
    ...
    // We now have exclusive access to either create or re-open an already existing shared memory segment
    SharedBuffer buffer;
    auto shared_buffer_result = buffer.Initialize(channel_params.channel_name, GetBufferSize(channel_params, element_size, element_alignment));
    ...
    if (header->m_consumer_count.load() == 0 && header->m_producer_count.load() == 0) {
        ...
        auto result = header->ring_buffer.Initialize(buffer.GetBuffer() + GetRingBufferSlotsOffset(element_alignment), element_size, element_alignment, channel_params.queue_size, true);
...
auto InterProcessConsumer::Receive(uint8_t* const destination_buffer, uint64_t destination_buffer_size) -> std::expected<void, PikaError>
{
...
auto InterProcessProducer::Send(uint8_t const* const source_buffer, uint64_t size) -> std::expected<void, PikaError>
{
