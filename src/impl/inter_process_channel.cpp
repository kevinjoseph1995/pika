...
static auto GetRingBufferSlotsOffset(uint64_t element_alignment) {
  ...
  if (element_alignment < sizeof(SharedBufferHeader)) {
    return ((sizeof(SharedBufferHeader) / element_alignment) + 1) *
           element_alignment;
  } else {
...
GetBufferSize(pika::ChannelParameters const &channel_parameters,
...
auto InterProcessConsumer::Create(pika::ChannelParameters const &channel_params,
  ...
  SharedBuffer buffer;
  auto shared_buffer_result = buffer.Initialize(
  ...
  if (header->m_consumer_count.load() == 0 &&
      header->m_producer_count.load() == 0) {
    ...
    auto result = header->ring_buffer.Initialize(
        buffer.GetBuffer() + GetRingBufferSlotsOffset(element_alignment),
        element_size, element_alignment, channel_params.queue_size, true);
    if (not result.has_value()) {
      return std::unexpected{result.error()};
    }
  }
  return std::unique_ptr<InterProcessConsumer>(
      new InterProcessConsumer(std::move(buffer)));
}
...
auto InterProcessConsumer::Receive(uint8_t *const destination_buffer,
  ...
  std::memcpy(destination_buffer, read_slot->GetElement(),
...
auto InterProcessProducer::Send(uint8_t const *const source_buffer,
                                uint64_t size)
    -> std::expected<void, PikaError> {
