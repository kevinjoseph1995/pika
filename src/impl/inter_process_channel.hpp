...
struct InterProcessConsumer : public pika::ConsumerImpl {
  static auto Create(pika::ChannelParameters const &channel_params,
  ...
  auto Receive(uint8_t *const destination_buffer,
               uint64_t destination_buffer_size)
      -> std::expected<void, PikaError> override;

private:
  InterProcessConsumer(SharedBuffer buffer) : m_buffer(std::move(buffer)) {}
...
private:
  InterProcessProducer(SharedBuffer buffer) : m_buffer(std::move(buffer)) {}
  auto getHeader() -> SharedBufferHeader & {
    PIKA_ASSERT(m_buffer.GetSize() != 0 && m_buffer.GetBuffer() != nullptr);
    return *reinterpret_cast<SharedBufferHeader *>(m_buffer.GetBuffer());
  }
