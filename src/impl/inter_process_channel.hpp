...
struct InterProcessConsumer : public pika::ConsumerImpl {
    static auto Create(pika::ChannelParameters const& channel_params, uint64_t element_size, uint64_t element_alignment) -> std::expected<std::unique_ptr<InterProcessConsumer>, PikaError>;
    auto Connect() -> std::expected<void, PikaError> override;
    auto Receive(uint8_t* const destination_buffer, uint64_t destination_buffer_size) -> std::expected<void, PikaError> override;
...
struct InterProcessProducer : public pika::ProducerImpl {
    static auto Create(pika::ChannelParameters const& channel_params, uint64_t element_size, uint64_t element_alignment) -> std::expected<std::unique_ptr<InterProcessProducer>, PikaError>;
    auto Connect() -> std::expected<void, PikaError> override;
    auto Send(uint8_t const* const source_buffer, uint64_t size) -> std::expected<void, PikaError> override;
