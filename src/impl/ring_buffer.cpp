...
auto RingBuffer::Initialize(uint8_t *ring_buffer, uint64_t element_size,
...
[[nodiscard]] auto RingBuffer::GetWriteSlot()
    -> std::expected<WriteSlot, PikaError> {
  ...
  m_header.not_full_condition_variable.Wait(
      locked_mutex_result.value(),
      [this]() -> bool { return m_header.count < m_max_number_of_elements; });
...
[[nodiscard]] auto RingBuffer::GetReadSlot()
    -> std::expected<ReadSlot, PikaError> {
  auto locked_mutex_result = LockedMutex::New(&m_header.mutex);
  if (not locked_mutex_result.has_value()) {
    return std::unexpected{locked_mutex_result.error()};
  }
...
WriteSlot::WriteSlot(WriteSlot &&other)
    : m_element(other.m_element),
      m_locked_mutex(std::move(other.m_locked_mutex)), m_cv(other.m_cv) {
  other.m_cv = nullptr;
