...
auto SharedBuffer::Initialize(std::string const &identifier, uint64_t size)
  ...
  if (stat.st_size != 0 &&
      stat.st_size != static_cast<decltype(stat.st_size)>(size)) {
    return std::unexpected(PikaError{
        .error_type = PikaErrorType::SharedBufferError,
        .error_message = fmt::format(
            "Shared memory object with identifier \"{}\" already exists;"
  ...
  if (stat.st_size == 0) {
    ...
    if (ret_code != 0) {
      ...
      return std::unexpected(PikaError{
          .error_type = PikaErrorType::SharedBufferError,
          .error_message =
              fmt::format("ftruncate failed with error:{}", error_message)});
    }
  }

  void *shared_memory_data =
      mmap(nullptr, size, PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0);
  if (*reinterpret_cast<int32_t *>(shared_memory_data) == -1) {
    return std::unexpected(PikaError{
        .error_message = fmt::format("mmap error: {}", strerror(errno))});
  }
...
SharedBuffer::~SharedBuffer() {
  if (m_data != nullptr) {
    ...
    if (result != 0) {
      auto error_message = strerror(errno);
      errno = 0;
      fmt::println(stderr, "munmap failed with error:{}", m_identifier,
                   error_message);
    }
    m_data = nullptr;
  }
  if (m_fd != -1) {
    ...
    if (result != 0) {
      auto error_message = strerror(errno);
      errno = 0;
      fmt::println(stderr, "shm_unlink({}) failed with error:{}", m_identifier,
                   error_message);
    }
    m_fd = -1;
  }
