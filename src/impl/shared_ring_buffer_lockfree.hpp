...
//             return ((sizeof(Header) / alignof(ElementType)) + 1) *
//             alignof(ElementType);
//         } else {
//             return alignof(ElementType);
//         }
//     }();
//     [[nodiscard]] auto Initialize(uint64_t number_of_elements) ->
...
//         auto const total_shared_buffer_size = RING_BUFFER_START_OFFSET +
//         (m_capacity * sizeof(ElementType)); auto result =
//         m_shared_buffer->Initialize(total_shared_buffer_size); if (not
//         result.has_value()) {
...
//     {
//         auto const current_tail =
//         getHeader().tail.load(std::memory_order_relaxed); auto const
//         next_tail = increment(current_tail); if (next_tail !=
//         getHeader().head.load(std::memory_order_acquire)) {
...
//     {
//         auto const current_head =
//         getHeader().head.load(std::memory_order_relaxed); if (current_head ==
//         getHeader().tail.load(std::memory_order_acquire)) {
//             return false;
//         }
//         element = getBufferSlot(current_head);
//         getHeader().head.store(increment(current_head),
...
//     {
//         PIKA_ASSERT(m_initialized);
//         return *reinterpret_cast<ElementType*>(m_shared_buffer->GetBuffer() +
//         RING_BUFFER_START_OFFSET + index * sizeof(ElementType));
//     }
