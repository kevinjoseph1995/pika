...
auto Semaphore::New(std::string const &semaphore_name, int32_t initial_value)
  ...
  if (semaphore_name.at(0) != '/') {
    return std::unexpected(PikaError{
        .error_type = PikaErrorType::SharedBufferError,
        .error_message =
            "Semaphore::Initialize: Semaphore name must begin with a \"/\""});
  }
  auto sem_ptr = sem_open(semaphore_name.c_str(), O_CREAT, S_IRUSR | S_IWUSR,
                          initial_value);
  if (sem_ptr == SEM_FAILED) {
    auto error_message = strerror(errno);
    errno = 0;
    return std::unexpected(
        PikaError{.error_type = PikaErrorType::SharedBufferError,
...
Semaphore::~Semaphore() {
  if (m_sem != nullptr) {
    ...
    if (ret != 0) {
      ...
      errno = 0;
      fmt::println(stderr,
                   "Semaphore::~Semaphore sem_unlink({}) failed with error {}",
...
auto Semaphore::Wait() -> void {
  ...
  if (sem_wait(m_sem) != 0) {
    auto error_message = strerror(errno);
    errno = 0;
    fmt::println(stderr, "Semaphore::Wait sem_wait failed with error {}",
                 error_message);
  }
}
...
auto Semaphore::Post() -> void {
  ...
  if (sem_post(m_sem) != 0) {
    auto error_message = strerror(errno);
    errno = 0;
    fmt::println(stderr, "Semaphore::Post sem_post failed with error {}",
                 error_message);
  }
}
...
auto Mutex::Initialize(bool intra_process) -> std::expected<void, PikaError> {
  ...
  if (return_code != 0) {
    return std::unexpected{PikaError{
        .error_type = PikaErrorType::SyncPrimitiveError,
        .error_message = fmt::format(
            "pthread_mutexattr_init failed with error code:{}", return_code)}};
  }
  if (intra_process) {
    return_code =
    ...
    if (return_code != 0) {
      return std::unexpected{PikaError{
          .error_type = PikaErrorType::SyncPrimitiveError,
          .error_message = fmt::format(
              "pthread_mutexattr_setpshared failed with error code:{}",
              return_code)}};
    }
  }
  ...
  if (return_code != 0) {
    return std::unexpected{PikaError{
        .error_type = PikaErrorType::SyncPrimitiveError,
        .error_message = fmt::format(
            "pthread_mutex_init failed with error code:{}", return_code)}};
  }
  m_initialized = true;
  return {};
}
...
Mutex ::~Mutex() {
  if (m_initialized) {
    ...
    if (return_code != 0) {
      fmt::println(stderr, "pthread_mutex_destroy failed with return code{}",
                   return_code);
    }
  }
}
...
auto Mutex::Lock() -> std::expected<void, PikaError> {
  ...
  if (return_code != 0) {
    return std::unexpected{PikaError{
        .error_type = PikaErrorType::SyncPrimitiveError,
        .error_message = fmt::format(
            "pthread_mutex_lock failed with return code:{}", return_code)}};
  }
  return {};
}
...
auto Mutex::Unlock() -> std::expected<void, PikaError> {
  ...
  if (return_code != 0) {
    return std::unexpected{PikaError{
        .error_type = PikaErrorType::SyncPrimitiveError,
        .error_message = fmt::format(
            "pthread_mutex_unlock failed with return code:{}", return_code)}};
  }
  return {};
}
...
LockedMutex::~LockedMutex() {
  if (m_mutex) {
    ...
    if (not result.has_value()) {
      fmt::println(
          stderr,
          "LockedMutex::~LockedMutex() Mutex unlock failed with error {}",
...
auto ConditionVariable::Initialize(bool intra_process)
  ...
  if (return_code != 0) {
    return std::unexpected{PikaError{
        .error_type = PikaErrorType::SyncPrimitiveError,
        .error_message = fmt::format(
            "pthread_condattr_init failed with error code:{}", return_code)}};
  }
  if (intra_process) {
    return_code =
    ...
    if (return_code != 0) {
      return std::unexpected{
          PikaError{.error_type = PikaErrorType::SyncPrimitiveError,
                    .error_message = fmt::format(
                        "pthread_condattr_setpshared failed with error code:{}",
                        return_code)}};
    }
  }
  ...
  if (return_code != 0) {
    return std::unexpected{PikaError{
        .error_type = PikaErrorType::SyncPrimitiveError,
        .error_message = fmt::format(
            "pthread_cond_init failed with error code:{}", return_code)}};
  }
  m_initialized = true;
  return {};
}
...
ConditionVariable ::~ConditionVariable() {
  if (m_initialized) {
    ...
    if (return_code != 0) {
      fmt::println(stderr, "pthread_cond_destroy failed with return code{}",
                   return_code);
    }
    m_initialized = false;
  }
