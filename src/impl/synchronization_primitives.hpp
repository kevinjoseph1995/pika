...
struct Semaphore {
  [[nodiscard]] static auto New(std::string const &semaphore_name,
...
struct Mutex {
  ...
  [[nodiscard]] auto Initialize(bool intra_process = false)
...
struct LockedMutex {
  [[nodiscard]] static auto New(Mutex *mutex)
      -> std::expected<LockedMutex, PikaError>;
  ~LockedMutex();
  LockedMutex(LockedMutex const &) = delete;
  LockedMutex(LockedMutex &&other) : m_mutex(other.m_mutex) {
...
private:
  friend struct ConditionVariable;
  LockedMutex(Mutex *mutex) : m_mutex(mutex) {}
...
struct ConditionVariable {
  ...
  void Wait(LockedMutex &locked_mutex, Predicate stop_waiting) {
    while (stop_waiting() == false) {
      auto status = pthread_cond_wait(&m_pthread_cond,
                                      &locked_mutex.m_mutex->m_pthread_mutex);
      if (status != 0) {
        fmt::println(stderr, "pthread_cond_wait failed with return code{}",
                     status);
        break;
      }
    }
